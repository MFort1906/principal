...
// ===== ALFA PRO CRONOGRAMA =====
window.importarCronograma = async function(event){
 const arquivo = event.target.files[0];
 if(!arquivo) return;
 const fd = new FormData();
 fd.append('arquivo', arquivo);

 const r = await fetch('/importar-cronograma',{
   method:'POST',
   body:fd
 });

 const data = await r.json();
 cronogramaPosts = data.dados || [];
 localStorage.setItem('alfa_cronograma', JSON.stringify(cronogramaPosts));
 if(typeof renderizarCronograma==='function') renderizarCronograma();
 alert('Cronograma importado com sucesso!');
};

window.gerarPostCronograma = async function(id){
 const post = cronogramaPosts[id];
 if(!post) return;

 adicionarMsgUsuario('Gerar conteúdo para: ' + post.tema);

 const prompt = `Crie um artigo de blog da Tennant, post LinkedIn, legenda Instagram e CTA para o tema: ${post.tema}`;
 ...
 const data = await r.json();
 adicionarMsgAlfa(data.texto || 'Conteúdo gerado.');
};
